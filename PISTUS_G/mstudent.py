...
def get_student(stud_number, cursor):
    ...
    T = ()
    try:
        query = "SELECT stud_number, first_name, last_name, gender FROM Student WHERE stud_number == ?"
        cursor.execute(query, (stud_number,))
        student_data = cursor.fetchone()
        
        if student_data:
            query2 = "SELECT email FROM EmailAddress WHERE stud_number == ? "
            cursor.execute(query2, (stud_number,))
            res = cursor.fetchall()
            T = student_data + (res)
...
def get_roles(cursor):
    ...
    ################ TODO: WRITE HERE THE CODE OF THE FUNCTION ##################
    try:
        query = "SELECT DISTINCT stud_role FROM member_of"
        cursor.execute(query)
        asso_roles = cursor.fetchall()
        student_roles = [role[0] for role in asso_roles]
...
def get_memberships(stud_number, cursor):
    ...
    ################ TODO: WRITE HERE THE CODE OF THE FUNCTION ##################
    try : 
        query = "SELECT asso_name, student_role FROM member_of WHERE stud_number = ?"
        cursor.execute(query, (stud_number,))
...
def test_add_email_address(cursor, conn):
    ...
    try:
        ...
        res = add_email_address(7719175, "eliane.choisne@centralesupelec.fr", cursor)
...
def test_add_student(cursor, conn):
    ...
    try: 
        ...
        cursor.execute("BEGIN")
        res = add_student(7719175, "Test", "Student", "F", ["test.student@centralesupelec.fr"], cursor)
        ...
        cursor.execute("BEGIN")
        res = add_student(1234, "Test", "Student", "F", ["eliane.choisne@centralesupelec.fr"], cursor)
        assert res == (False, DUPLICATE_EMAIL_ADDRESS, "eliane.choisne@centralesupelec.fr"), "The email address eliane.choisne@centralesupelec.fr is already in use. \
            The function should return (False, DUPLICATE_EMAIL_ADDRESS, eliane.choisne@centralesupelec.fr)"
        conn.rollback()
        ...
        cursor.execute("BEGIN")
        res = add_student(1234, "Test", "Student", "F", ["test.student@centralesupelec.fr", "test.student@gmail.com"], cursor)
...
def test_delete_email_address(cursor, conn):
    ...
    try:
        cursor.execute("BEGIN")
        res = delete_email_address(6655783, "zineb.algourdin@centralesupelec.fr", cursor)
...
def test_update_email_address(cursor, conn):
    ...
    try:
        ...
        cursor.execute("BEGIN")
        res = update_email_address(3528, "ericka.guyomard@centralesupelec.fr", "eliane.choisne@centralesupelec.fr", cursor)
        ...
        cursor.execute("BEGIN")
        res = update_email_address(3528, "ericka.guyomard@centralesupelec.fr", "ericka.guyomard@gmail.com", cursor)
