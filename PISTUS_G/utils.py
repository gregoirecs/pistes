...
def load_config():
    ...
    with open("./config/config",'r') as csv_file:
...
def load_messages_bundle(messages_bundle_file):
    ...
    ################ TODO: WRITE HERE THE CODE OF THE FUNCTION ##################
    with open(messages_bundle_file) as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=",")
        #next(csv_reader)
        for row in csv_reader:
            key, value = row
    ...
    ##############################################################################
print(load_messages_bundle('./config/messages_bundle_fr'))
    ...
    * Password must contain at least one symbol character (@,!,#,$)
    ...
    for k in P:
        if k in Capital:
            ...
            SC = True
    if len(password)> 4 and CL == True and D == True and SC == True:
        return True
    else:
        return False
